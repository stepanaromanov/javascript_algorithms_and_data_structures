...
function palindrome(str) {
    ...
    const newstr = str.toLowerCase()
                      .split(/[ \-*#_,:./\\()]/)
                      .join("");
    const odd = newstr.length % 2 === 1 ? true : false;
    return odd ? (newstr.slice(0, newstr.length / 2) === reverse(newstr.slice(newstr.length / 2  + 1, newstr.length))) 
               : (newstr.slice(0, newstr.length / 2) === reverse(newstr.slice(newstr.length / 2, newstr.length)));
}